'''
Created on May 27, 2017

@author: ayush

John has discovered various rocks. Each rock is composed of various elements, and each element is represented by a lower-case Latin letter from 'a' to 'z'. An element can be present multiple times in a rock. An element is called a gem-element if it occurs at least once in each of the rocks.

Given the list of rocks with their compositions, display the number of gem-elements that exist in those rocks.

Input Format

The first line consists of an integer, , the number of rocks.
Each of the next lines contains a rock's composition. Each composition consists of lower-case letters of English alphabet.

Constraints

Each composition consists of only lower-case Latin letters ('a'-'z').
length of each composition

Output Format

Print the number of gem-elements that are common in these rocks. If there are none, print 0.

Sample Input

3
abcdde
baccd
eeabg

Sample Output

2

Explanation

Only "a" and "b" are the two kinds of gem-elements, since these are the only characters that occur in every rock's composition.
'''

n=int(input("Enter the number of inputs"))
lst=[] #'abcdde','baccd','eeabg','abdcv','abc'
for i in range(n):
    s=input()
    lst.append(s)
# lst2=[]
# lst3=[]
# st=lst[0]
# flag=0
# if(len(lst)!=1):
#     st2=lst[1]
# if(len(lst)==1):
#     print(len(lst))
#     
# for character in st:
#     if character in st2 and character not in lst2:
#         lst2.append(character)
# 
# for j in range(2,len(lst)):
#     if(flag==1):
#         lst2=lst3
#         lst3=[]
#     for character in lst2:
#         if character in lst[j] and character not in lst3:
#             lst3.append(character)
#             flag=1
# print(len(lst3))  

#Alternate solution
#rocks = ['abcde','afsdsadb','dsadsadaab']
#print(rocks)
lst2=[]
for i in lst:
    a=set(i)
    #print(a)
    lst2.append(a)
#print(lst2)
c=set.intersection(*lst2)
print(len(c))